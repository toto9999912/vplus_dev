...
class UploadProgressController {
  ...
  final ValueNotifier<int> _completedFiles = ValueNotifier(0);
  final ValueNotifier<int> _totalFiles = ValueNotifier(0);
  ...
  // 獲取已完成的文件數量
  int get completedFiles => _completedFiles.value;

  // 獲取總文件數量
  int get totalFiles => _totalFiles.value;

  // 設定對話框參考
  void _setDialog(AwesomeDialog dialog) {
    _dialog = dialog;
  }
  ...
  // 更新已完成的文件計數
  void updateCompletedFiles(int completed) {
    _completedFiles.value = completed;
  }
  ...
  // 同時更新進度和訊息
  void update({double? progress, String? message, int? completedFiles, int? totalFiles}) {
    if (progress != null) {
...
class DialogService {
  ...
  UploadProgressController showMediaUploadDialog({
    String title = '上傳媒體',
    String initialMessage = '準備上傳中...',
    ...
    controller._progress.value = initialProgress.clamp(0.0, 1.0);
    controller._message.value = initialMessage;
    controller._totalFiles.value = totalFiles;
    controller._completedFiles.value = 0;
    ...
    final dialog = AwesomeDialog(
      ...
      dismissOnTouchOutside: dismissOnTouchOutside,
      dismissOnBackKeyPress: dismissOnTouchOutside,
      btnCancelText: showCancelButton ? cancelButtonText : null,
      btnCancelOnPress:
          showCancelButton
              ? () {
                if (onCancel != null) {
        ...
        child: Column(
          ...
          children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        ValueListenableBuilder<int>(
                          valueListenable: controller._completedFiles,
                          builder: (context, completed, _) {
                            return ValueListenableBuilder<int>(
                              valueListenable: controller._totalFiles,
                              builder: (context, total, _) {
                                if (total > 0) {
                                  return Text('$completed/$total', style: const TextStyle(fontSize: 14));
                                }
            ...
            ValueListenableBuilder<String>(
              valueListenable: controller._message,
              builder: (context, message, _) => Text(message, textAlign: TextAlign.center, style: const TextStyle(fontSize: 14)),
            ),
    ...
    final dialog = AwesomeDialog(
      ...
      body: Padding(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            customLoadingWidget ?? CircularProgressIndicator(color: loadingColor ?? Theme.of(_ctx).colorScheme.primary, strokeWidth: 4.0),
            const SizedBox(height: 16),
